...
let urRoute = {
  /*預設  856 去程*/ url1Text: `${routename}?$top=1`,
  stopFrom: 0,
  stopEnd: 14,
  RouteName: routename
};
...
function linkStopOrder(dealData) {
  //取得站名排列順序
  stopIdOrder = [];
...
function linkBusPos(dealData) {
  //取得公車目前位置
  busPosition = [];
  for (let i = 0; i < dealData.length; i++) {
    if (
      dealData[i].RouteName.Zh_tw == urRoute.RouteName &&
      dealData[i].Direction == direction
    ) {
      busPosition.push({
        PlateNumb: dealData[i].PlateNumb,
        StopName: dealData[i].StopName.Zh_tw,
        A2EventType: dealData[i].A2EventType
      });
    }
  }
  dynamicBus(); //變
...
function dynamicBus() {
  ...
  for (
    let i = 0;
    i < busPosition.length && document.body.offsetWidth > 991;
    i++
  ) {
    ...
    if (busCurrentStopIndex != -1) {
      ...
      if (i != stopIdOrder.length - 1) {
        //最後一班離開不顯示
        document.getElementById(
          `eta${io}${busCurrentStopIndex}`
        ).innerHTML = `<span class="bustitle">${busPosition[i].PlateNumb}</span>`;
