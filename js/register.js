  ...
  methods: {
    ...
    testId() {
      if (/[\u4e00-\u9fa5]/.test(this.memId)) {
        ...
        fetch(`php/login/register.php?id=${this.memId}`)
          .then((res) => res.text())
          .then((result) => {
      ...
      if (err > 0) {
        ...
        xhr.onload = function () {
          if (xhr.status == 200) {
            ...
            $id(
              "headerMemName"
            ).innerHTML = `${userData.memName}<a id="logout" href="javascript:;">登出</a>`;
